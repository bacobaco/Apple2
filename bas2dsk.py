...
def write_file_to_dsk(dsk_path, filename, file_data, file_type):
    ...
    # Save disk image
    # Create backup in scratch/ folder if available to keep root clean
    base_dir = os.path.dirname(os.path.abspath(dsk_path))
    scratch_dir = os.path.join(base_dir, "scratch")
    if os.path.isdir(scratch_dir):
        backup_path = os.path.join(scratch_dir, os.path.basename(dsk_path) + ".bak")
    else:
...
def main():
    ...
    if ext == '.bin':
        ...
        # If binary already starts with 2-byte load address matching load_addr, strip it
        if len(raw_data) >= 2 and raw_data[0] == (load_addr & 0xFF) and raw_data[1] == (load_addr >> 8):
            raw_data = raw_data[2:]
